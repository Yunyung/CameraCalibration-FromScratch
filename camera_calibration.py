...
"""
homographies = [] # Homography matrix of each image

for i in range(len(imgpoints)): # Find out the homography matrix of each iamge
    homographies.append(feature.solve_homography(objp, imgpoints[i]))
    
intrinsic, B = feature.solve_intrinsic(homographies) # Use homographies to find out the intrinsic matrix K  
extrinsic_nonfunc = feature.solve_extrinsics(intrinsic, homographies) # Find out the extrinsics of each images by intrinsic and homographies   
extrinsics = extrinsic_nonfunc 
